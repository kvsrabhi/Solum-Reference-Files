...
####################### User Defined start #######################
my $base_path = "/home/common00/cloud_common00";
my $base_version_string = "1.1";
...
#my $dboperator_yaml = "$base_path/dboperator.yaml";
...
{
        print ("\n\n");
        print color(yellow),("Base Version change happens once in a day (Ex: 1.0.27  -> only '27' will be changed to '28') \n");
        ...
        while ($valid_input)
        {
                ...
                print "\n\nPress", color(green), " 'Y' ", color("reset"), "to change base version or", color(red), " 'N' ", color("reset"), "to continue latest deployment: ";

                $user_input = <STDIN>;
                chomp $user_input;
                $user_input =~ tr/ //ds; #Removing white spaces

                if (uc($user_input) eq "Y")
                {
                        $existing_base_string = read_base_version_from_yaml($api_yaml);
                        print "\nCurrent Base Version:", color(green), " $existing_base_string \n", color("reset");

                        my @splitted_version = split('\.', $existing_base_string);
                        my $last_tag = $splitted_version[2];

                        $last_tag= $last_tag + 1;
                        my $new_base_string = $base_version_string.".".$last_tag; 

                        print "\nNew Base Version will be:", color(green), " $new_base_string \n\n", color("reset");

                        print "\n\nPress", color(green), " 'Y' ", color("reset"), "to confirm or", color(red), " 'N' ", color("reset"), "to exit: ";

                        $user_input_1 = <STDIN>;
                        chomp $user_input_1;
                        $user_input_1 =~ tr/ //ds; #Removing white spaces

                        if (uc($user_input_1) eq "Y")
                        {
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $api_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $apiaqp_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $ig_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $inbound_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $outbound_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $scheduler_yaml >/dev/null 2>/dev/null");
                                #system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $dboperator_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $dash_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $lbs_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $ld_yaml >/dev/null 2>/dev/null");
                                #system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $ikea_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $realtime_yaml >/dev/null 2>/dev/null");
                                system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $square_yaml >/dev/null 2>/dev/null");
                                #system("perl -pi.back -e 's/$existing_base_string/$new_base_string/g;' $pickcel_yaml >/dev/null 2>/dev/null");

                                print "\nSuccessfully updated Base Version in all yamls files !!\n\n";
                        }
                        elsif (uc($user_input_1) eq "N")
                        {
                                exit;
                ...
                elsif(uc($user_input) eq "N")
                {
                        ...
                        if ( $check == 0 )
                        {
                                print color(red),"There is no recent pipelines ran.\n", color("reset");
                        }
        ...
        if ( $latest_tag != $existing_tag )
        {
                ...
                if ( $result == 0)
                {
                        ...
                        if ( $acr_img eq $DASH )
                        {
                            ...
                            system("kubectl delete -f $api_yaml >/dev/null 2>/dev/null");
                            sleep(90);
                            system("kubectl apply -f $api_yaml >/dev/null 2>/dev/null");
                        }
                        ...
                        elsif ( $acr_img eq $LD )
                        {
                            ...
                            # Positon of below elements are fixed in yaml file.
                            # Any changes to position of 'env' elements in yaml, below indices (4,5) must be adjusted accordingly
                        ...
                        system("kubectl delete -f $yaml_file >/dev/null 2>/dev/null");
                        sleep(60);
                        system("kubectl apply -f $yaml_file >/dev/null 2>/dev/null");
                        sleep(30);

                        print color(green), "\nSuccessfully deployed '$yaml_file' with updated acr tag: $latest_tag !! \n", color("reset");
                        print "\n-------------------------------\n\n";
                }
        }
}
